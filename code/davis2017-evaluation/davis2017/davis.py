...
class DAVIS(object):
    SUBSET_OPTIONS = ['train', 'val', 'test-dev', 'test-challenge', 'trainval']
    ...
    def __init__(self, root, task='unsupervised', subset='val', sequences='all', resolution='480p', codalab=False, year='2017'):
        ...
        self.imagesets_path = os.path.join(self.root, 'ImageSets', resolution) if year == '2016' else os.path.join(self.root, 'ImageSets', year)
        ...
        if sequences == 'all':
            with open(os.path.join(self.imagesets_path, f'{self.subset}.txt'), 'r') as f:
                tmp = f.readlines()
            sequences_names = sorted({x.split()[0].split('/')[-2] for x in tmp}) if year == '2016' else [x.strip() for x in tmp]
